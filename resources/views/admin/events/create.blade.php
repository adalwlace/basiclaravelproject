@extends('admin.layout.master')
@section('content')
    <h1>Create</h1>
    <form action="{{ route('admin.events.add') }}" method="POST">
        <div style="display: flex; flex-direction:column; padding:20px">

            @csrf
            <label for="event_name">Event Name</label>
            <input type="text" name="event_name" id="" style="padding:20px">

            <label for="date">Date</label>
            <input type="text" name="date" id="" style="padding: 20px">

            <label for="location">Location</label>
            <input type="text" name="location" id="" style="padding:20px">

            <button
                style="width: 10%; margin-top:5px; padding: 10px 20px; background-color: #007bff; color: white; border:none; border-radius: 5px;">Create</button>
        </div>
    </form>
@endsection
